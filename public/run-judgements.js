...
function parseCsv(text) {
  ...
  if (!lines.length) return { topics: [], personaIds: null };

  // Optional first line: "personas: machiavelli, nietzsche, mill"
  let personaIds = null;
  let rest = lines;
  if (lines[0].toLowerCase().startsWith("personas:")) {
    const ids = lines[0].slice("personas:".length)
...
function buildHtml(topics, results, date, personas, portraits) {
...
function buildIndex(scriptDir) {
  ...
  const entries = files.map((f, i) => {
    ...
    console.log(`  Index: ${f} → ${questions.length} question(s), ${cardCount} cards, maxim: ${maxim ? "✓" : "✗"}`);

    return { file: f, issue, displayDate, questions, maxim, attribution, personaCount };
  });
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .idx-issue { font-size: 0.6rem; letter-spacing: 0.18em; text-transform: uppercase; color: var(--gold); margin-bottom: 0.75rem; }
...
(async () => {
  ...
  const { topics, personaIds } = parseCsv(fs.readFileSync(inputFile, "utf8"));
  if (!topics.length) {
  ...
  // Write HTML
  fs.writeFileSync(htmlFile, buildHtml(topics, results, date, activePersonas, portraits), "utf8");
